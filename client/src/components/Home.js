    ...
    <div className="animate-fadeIn -mx-6">
      ...
      <section className="relative overflow-hidden min-h-[70vh] flex flex-col items-center justify-center px-6 text-center"
        ...
        <div className="absolute top-32 right-[5%] w-20 h-20 md:w-28 md:h-28 rounded-[2rem] overflow-hidden shadow-2xl border-4 border-white rotate-[6deg] animate-float hidden md:block" style={{ animationDelay: "1s" }}>
          <img src={products[10]?.image} alt="" className="w-full h-full object-cover" onError={(e) => { e.target.src = "https://placehold.co/200x200?text=🎀"; }} />
        </div>
        <div className="absolute bottom-32 left-[8%] w-20 h-20 md:w-24 md:h-24 rounded-[2rem] overflow-hidden shadow-2xl border-4 border-white rotate-[5deg] animate-float hidden md:block" style={{ animationDelay: "2s" }}>
          <img src={products[31]?.image} alt="" className="w-full h-full object-cover" onError={(e) => { e.target.src = "https://placehold.co/200x200?text=💐"; }} />
        </div>
        <div className="absolute bottom-20 right-[8%] w-28 h-28 md:w-36 md:h-36 rounded-[2rem] overflow-hidden shadow-2xl border-4 border-white rotate-[-5deg] animate-float hidden md:block" style={{ animationDelay: "0.5s" }}>
          <img src={products[45]?.image} alt="" className="w-full h-full object-cover" onError={(e) => { e.target.src = "https://placehold.co/200x200?text=👜"; }} />
        </div>

        <div className="relative z-10 max-w-3xl">
          <div className={`inline-flex items-center gap-2 backdrop-blur-md border px-5 py-2 rounded-full mb-8 shadow-sm ${dark ? 'bg-purple-950/60 border-purple-800' : 'bg-white/80 border-purple-100'}`}>
            <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse" />
            <span className="text-[10px] font-black text-purple-600 uppercase tracking-[0.3em]">Handcrafted in India 🇮🇳</span>
          </div>
          <h1 className={`font-black leading-tight tracking-tighter mb-6 ${dark ? 'text-white' : 'text-gray-900'}`} style={{ fontSize: "clamp(2rem, 5vw, 3.5rem)" }}>
          ...
          <div className="flex flex-wrap justify-center gap-4 mt-12">
            {[{ icon: "🧶", text: "100% Handmade" }, { icon: "🚚", text: "Free Ship ₹500+" }, { icon: "🎁", text: "Free Gift ₹300+" }, { icon: "💜", text: "Made to Order" }].map((b, i) => (
              <div key={i} className={`flex items-center gap-2 backdrop-blur-sm px-4 py-2 rounded-full border shadow-sm ${dark ? 'bg-purple-950/60 border-purple-800' : 'bg-white/80 border-purple-50'}`}>
                <span className="text-sm">{b.icon}</span>
                <span className={`text-[9px] font-black uppercase tracking-widest ${dark ? 'text-purple-200' : 'text-gray-600'}`}>{b.text}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── MARQUEE ───────────────────────────────────────────── */}
      <div className={`overflow-hidden py-3 border-y backdrop-blur-sm ${dark ? 'border-purple-900 bg-purple-950/40' : 'border-purple-100 bg-white/60'}`}>
              ...
              {newArrivals.map((prod) => (
                <div key={prod.id} onClick={() => { setSelectedProduct(prod); setView('product-detail'); window.scrollTo(0, 0); }}
                  ...
                  <div className="mt-3 px-1">
                    <h4 className={`font-black text-sm truncate ${dark ? 'text-purple-100' : 'text-gray-800'}`}>{prod.name}</h4>
                    <p className="text-purple-500 font-black italic">₹{prod.price}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {HOME_CATEGORIES.slice(0, 2).map((cat) => {
              ...
              return (
                <div key={cat.id}
                  ...
                  className={`rounded-[3rem] border-4 shadow-xl hover:-translate-y-2 hover:shadow-2xl transition-all duration-300 cursor-pointer group overflow-hidden relative p-7 ${dark ? 'bg-gradient-to-br from-purple-950/80 to-gray-900/80 border-purple-800/50' : `bg-gradient-to-br ${cat.gradient} border-white`}`}
          ...
          <div className="grid grid-cols-3 gap-5">
            {HOME_CATEGORIES.slice(2, 5).map((cat) => {
              ...
              return (
                <div key={cat.id}
                  onClick={() => { setActiveCat(cat.id); setView('subcat'); window.scrollTo(0, 0); }}
                  className={`rounded-[3rem] border-4 shadow-xl hover:-translate-y-2 hover:shadow-2xl transition-all duration-300 cursor-pointer group overflow-hidden relative p-6 ${dark ? 'bg-gradient-to-br from-purple-950/80 to-gray-900/80 border-purple-800/50' : `bg-gradient-to-br ${cat.gradient} border-white`}`}
                  style={{ minHeight: "260px" }}
                >
                  ...
                  <h3 className={`font-black text-base tracking-tighter leading-tight ${dark ? 'text-white' : 'text-gray-900'}`}>{cat.name}</h3>
                  <p className={`text-[9px] font-medium mt-1 mb-5 ${dark ? 'text-purple-300' : 'text-gray-500'}`}>{cat.desc}</p>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {count > 0 && (
                      <span className={`text-[8px] font-black ${dark ? 'text-purple-400' : 'text-gray-400'}`}>{count} items</span>
                    )}
                  </div>
                </div>
              );
